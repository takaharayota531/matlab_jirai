...
function [s_list,h_list,f_list,K_list]=atodekesu(S,model,p,alpha_size)
    K=S;
    h=calc_h(K,model);
...
%     df_list = zeros(Nx,Ny,FREQ_POINT*4,2,0);
    ...
    %   cc="順番超変更"
        ...
        %                 tmp_K=S;
        %                 [tmp_f,~]=calc_h_and_f(tmp_K,model,p);
        %                 alpha=complex_random_array(alpha_size,Nx,Ny);
        %                 can_S=S;
        %                 can_S(:,:,k)=S(:,:,k)+alpha;
        %                 can_K=can_S;
        %                 [can_f,can_h]=calc_h_and_f(can_K,model,p);
...
function complex_random=complex_random_array(alpha_size,Nx,Ny)
    a = -alpha_size;
    b = alpha_size;
    re = (b-a).*rand(Nx,Ny) + a;
    im = (b-a).*rand(Nx,Ny) + a;
