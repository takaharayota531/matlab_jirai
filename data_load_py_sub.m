...
function [s_use,f] = data_load_py_sub(dataname,datanameH)
disp('Data loading ...');
tic
...
movSubFilter=1/9*[-1 -1 -1;-1  8 -1;-1 -1 -1];

 for i=1:Nf
     data(:,:,i,1)=uint8(filter2(movSubFilter,data(:,:,i,1)));
    data(:, :, i, 2) = uint8(filter2(movSubFilter, data(:, :, i, 2)));
 end
